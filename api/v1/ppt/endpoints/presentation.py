...
import dirtyjson
from utils.filename_generator import generate_presentation_filename
...
async def stream_presentation(
    ...
    await consume_credits(sql_session, user_id, "presentation_generate")
    presentation = await sql_session.get(PresentationModel, id)
    if not presentation:
        raise HTTPException(status_code=404, detail="Presentation not found")
...
async def update_presentation(
    ...
    if not data.get("skip_credit_charge", False):
        user_id = get_request_user_id(request)
        await consume_credits(sql_session, user_id, "presentation_update")
    # Extract the presentation ID from the data
    presentation_id = data.get("id") or data.get("presentation_id")
    if not presentation_id:
        raise HTTPException(status_code=400, detail="Presentation ID is required")
    ...
    # Update presentation fields if provided
    if "n_slides" in data and data["n_slides"]:
        presentation_update_dict["n_slides"] = data["n_slides"]
    if "title" in data and data["title"]:
        presentation_update_dict["title"] = data["title"]
    ...
    if "slides" in data and data["slides"]:
        slides = data["slides"]
        # Convert slides to SlideModel objects if they're dicts
        slide_models = []
        for slide in slides:
            if isinstance(slide, dict):
                # Convert string UUIDs to UUID objects
                if "presentation" in slide and isinstance(slide["presentation"], str):
                    slide["presentation"] = uuid.UUID(slide["presentation"])
                if "id" in slide and isinstance(slide["id"], str):
                    slide["id"] = uuid.UUID(slide["id"])
                try:
                    slide_models.append(SlideModel(**slide))
                except Exception as e:
                    print(f"Error converting slide to SlideModel: {e}")
                    continue
            else:
                slide_models.append(slide)
...
async def export_presentation_as_pptx(
    ...
    try:
        # Validate the presentation model before processing
        if not pptx_model.slides or len(pptx_model.slides) == 0:
            raise HTTPException(status_code=400, detail="Cannot export empty presentation")
        
        temp_dir = TEMP_FILE_SERVICE.create_temp_dir()

        print(f"Creating PPTX with {len(pptx_model.slides)} slides")
        pptx_creator = PptxPresentationCreator(pptx_model, temp_dir)
        await pptx_creator.create_ppt()

        export_directory = get_exports_directory()
        
        # Use provided name if available, otherwise generate a default
        if pptx_model.name:
            base_name, ext = os.path.splitext(pptx_model.name)
            filename = (
                pptx_model.name
                if ext.lower() == ".pptx"
                else f"{(base_name or pptx_model.name)}.pptx"
            )
        else:
        ...
        if file_size == 0:
            raise HTTPException(status_code=500, detail="Generated PPTX file is empty")
            
        print(f"PPTX export successful: {filename} ({file_size} bytes)")
        return f"/app_data/exports/{quote(filename)}"
        
    except Exception as e:
        print(f"Error in PPTX export: {e}")
        import traceback
        traceback.print_exc()
