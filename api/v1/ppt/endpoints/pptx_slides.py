...
async def process_pptx_slides(
    ...
    4. Optionally uses LibreOffice to generate slide screenshots
    5. Falls back to static placeholder screenshots when native rendering is disabled/unavailable
    6. Returns screenshot URLs and XML content for each slide
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if True:
            ...
            # Convert PPTX to PDF only when explicitly enabled for this deployment.
            pdf_path: Optional[str] = None
            if _is_libreoffice_render_enabled():
                pdf_path = await _convert_pptx_to_pdf(pptx_path, temp_dir)
            else:
                print("Skipping LibreOffice rendering (ENABLE_LIBREOFFICE_PPTX_RENDER is disabled)")

            # Generate screenshots using LibreOffice PDF
            if pdf_path:
                ...
                # Fallback: No LibreOffice/native render, use per-slide placeholders.
                print("Using placeholder screenshots for PPTX slides")
                screenshot_paths = []
            ...
            for i, xml_content in enumerate(slide_xmls, 1):
                ...
                screenshot_path = screenshot_paths[i - 1] if i - 1 < len(screenshot_paths) else None
...
def _is_libreoffice_render_enabled() -> bool:
    """
    Controls whether native LibreOffice rendering is allowed for PPTX screenshots.
    Default is disabled to keep the flow fully web-safe.
    """
    return os.getenv("ENABLE_LIBREOFFICE_PPTX_RENDER", "false").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
async def _install_fonts(fonts: List[UploadFile], temp_dir: str) -> None:
    ...
    for font_file in fonts:
        # Save font file
        font_path = os.path.join(fonts_dir, font_file.filename or "unknown_font")
...
async def _convert_pptx_to_pdf(pptx_path: str, temp_dir: str) -> Optional[str]:
    ...
    try:
        ...
        # Step 1: Convert PPTX to PDF using LibreOffice (optional)
        ...
        libreoffice_paths = [
            "libreoffice",  # Command line version
            "/Applications/LibreOffice.app/Contents/MacOS/soffice",  # macOS app
            "/usr/bin/libreoffice",  # Linux system
        ]
        ...
        if not libreoffice_cmd:
            print("⚠️  LibreOffice not found. To enable PDF conversion, install it:")
            print("   macOS: brew install libreoffice")
            print("   Linux: sudo apt-get install libreoffice")
            print("   Continuing with PPTX processing...")
            return None
        ...
        try:
            ...
            print("   Linux: sudo apt-get install libreoffice")
            print("   Continuing with PPTX processing...")
            # Return None to indicate PDF conversion skipped
            return None
        ...
        if "libreoffice" in str(e):
            return None  # Allow continuation without PDF
        raise
    except Exception as e:
